...
# --- SAFE VALUE EXTRACTION HELPER (FIXES SERIES TRUTH VALUE AMBIGUITY) ---
...
if up_fin and st.sidebar.button("Save & Auto-Populate Financial Data"):
    try:
        ...
        df_u = df_u.fillna('')  # Safe conversion of NaN values
        
        conn = get_connection()
        c = conn.cursor()
        
        cols = [str(col).strip().upper() for col in df_u.columns]
        ...
        for idx, row in df_u.iterrows():
            ...
            d_val = safe_str(row_dict.get("DATE"))
            if not d_val:
            ...
            # Auto-populate to Petty Cash module if marked
        ...
        conn.commit()
        conn.close()
        st.sidebar.success("Financial Ledger uploaded & populated to P&L, VAT, Tax, Salary, and Petty Cash modules!")
        st.rerun()
...
if nav == "Overview & Financial Statements":
    ...
    cash_df = df_fin[(df_fin["payment_mode"].str.upper().str.contains("CASH", na=False)) | (df_fin["is_petty_cash"].str.upper() == "YES")] if not df_fin.empty else pd.DataFrame()
    cash_bal = (cash_df["income_net"].sum() - cash_df["expense_net"].sum()) if not cash_df.empty else 0.0
    ...
    with col_b:
        ...
        if not df_quot.empty:
            st.dataframe(df_quot[["quotation_id", "client_name", "quotation_amount", "feedback_status"]], use_container_width=True)
        else:
...
# --- MODULE 2: DATA IMPORT, EXPORT & CLEAR ---
    ...
    with t1:
        st.download_button("Financial Template (.xlsx)", generate_financial_template(), "Financial_Template.xlsx")
    with t2:
        st.download_button("Project Template (.xlsx)", generate_project_template(), "Project_Template.xlsx")
    with t3:
        st.download_button("Quotation Template (.xlsx)", generate_quotation_template(), "Quotation_Template.xlsx")
    with t4:
        st.download_button("Petty Cash Template (.xlsx)", generate_petty_cash_template(), "Petty_Cash_Template.xlsx")

    st.markdown("---")
    st.markdown("### 2. Manual Data Entry Forms")
    tab_f, tab_p, tab_pc, tab_q = st.tabs(["Financial Transaction", "New Project", "Petty Cash Transaction", "Quotation Proposal"])
            ...
            if st.form_submit_button("Save & Populate System"):
                ...
                if ft_is_petty == "YES" or ft_mode == "Cash":
                    c.execute('''
                        INSERT INTO petty_cash (sl_no, date, voucher_no, description, cash_in, cash_out, balance, remarks)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (1, str(ft_date), ft_bill, ft_part, 
                          ft_net if ft_type == "Income" else 0.0,
                          ft_net if ft_type == "Expense" else 0.0, 0.0, "Manual Entry - Auto Populated"))
    ...
    with tab_q:
        with st.form("q_add_form"):
            ...
            q_status = st.selectbox("Feedback Status", ["In Process", "Closed - Won", "Closed - Lost"])
            ...
            if st.form_submit_button("Save Proposal"):
                ...
                ''', (f"Q-{datetime.datetime.now().strftime('%M%S')}", q_cname, q_pname, str(q_date), str(q_close), str(q_rem), q_amt, q_status, q_notes))
                conn.commit()
                conn.close()
...
# --- MODULE 3: P&L (YOY ANALYSIS) ---
    ...
    if not df_fin.empty:
        ...
        with f_col1:
            modes = st.multiselect("Filter Payment Modes", options=df_fin["payment_mode"].unique(), default=df_fin["payment_mode"].unique())
        with f_col2:
            petty_filter = st.multiselect("Include Petty Cash Records", options=df_fin["is_petty_cash"].unique(), default=df_fin["is_petty_cash"].unique())
            
        df_filtered = df_fin[(df_fin["payment_mode"].isin(modes)) & (df_fin["is_petty_cash"].isin(petty_filter))].copy()
        
        df_filtered["Year"] = df_filtered["date_dt"].dt.year
        years = sorted([int(y) for y in df_filtered["Year"].dropna().unique() if y >= 2024])
        ...
        if years:
            ...
            del_id = st.number_input("Enter Record ID to delete from Financials", min_value=1, step=1)
...
# --- MODULE 4: VAT & CORPORATE TAX RETURNS ---
    ...
    tab1, tab2 = st.tabs(["1. Corporate Tax Assessment (Jan to Dec)", "2. VAT Quarter-on-Quarter Returns"])
    ...
    with tab2:
        ...
        if not df_fin.empty:
            ...
            if vat_years:
                v1, v2 = st.columns(2)
                with v1:
                    vy = st.selectbox("Select Year", vat_years, index=len(vat_years)-1, key="vy_sel")
                with v2:
                    q_opt = st.selectbox("Select Custom VAT Quarter", [
...
# --- MODULE 6: PROJECT PROFITABILITY ---
    ...
    if not df_p.empty:
        st.metric("Total Contract Value", f"AED {df_p['total_amount'].sum():,.2f}")
    ...
    if not df_q.empty:
        ...
        for idx, row in df_q.iterrows():
            with st.expander(f"ID: {row['id']} | Ref: {row['quotation_id']} - {row['client_name']} (AED {row['quotation_amount']:,.2f})"):
                u1, u2 = st.columns(2)
                with u1:
                    new_st = st.selectbox("Status", ["In Process", "Closed - Won", "Closed - Lost"], 
                                          index=["In Process", "Closed - Won", "Closed - Lost"].index(row["feedback_status"]), key=f"st_{row['id']}")
                    new_amt = st.number_input("Amount (AED)", value=float(row["quotation_amount"]), key=f"am_{row['id']}")
                with u2:
                    new_rem = st.date_input("Reminder Date", value=pd.to_datetime(row["followup_reminder_date"]).date(), key=f"dt_{row['id']}")
                    new_notes = st.text_area("Notes", value=str(row["notes"]), key=f"nt_{row['id']}")
...
# --- MODULE 8: STAFF SALARIES TRACKER ---
    ...
    tab_s1, tab_s2 = st.tabs(["1. Salary Calculator & Ledger Disbursements", "2. Manage Employee Agreements"])
    ...
    with tab_s2:
        ...
        prof_del = st.number_input("Enter Employee Profile ID to delete", min_value=1, step=1)
    ...
    with tab_s1:
        ...
        if names_list:
            ...
            with sc1:
                monthly_val = st.number_input(f"Contract Monthly Salary for {sel_emp} (AED)", min_value=0.0, value=base_sal)
                m_cnt = st.number_input("Entitled Tenure Months", min_value=1, value=months_cnt)
            ...
            st.info("No employee salary transactions found. Add employee contracts to calculate.")
...
# --- MODULE 9: DOCUMENT GENERATOR ---
    ...
    with c_b:
        amt_base = st.number_input("Base Amount (Excl VAT AED)", min_value=0.0, value=10000.0)
